...
#Função responsável por gerar o arquivo HTML de assinatura de e-mail
function outlookSign {
    $cred = Get-Credential                                                                                               #Armazena na variável $cred os dados de acesso fornecidos pelo usuário mediante ao PopUp exibido
    Connect-MsolService -Credential $cred -ErrorAction Stop                                                              #Estabelece conexão com o Office365

    $list = Get-MsolUser -UserPrincipalName DOMINIO_SUA_EMPRESA -All                                                     #Lista todos os usuários cadastrado no Office365 no respectivo domínio da sua empresa
